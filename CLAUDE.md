...
| #8 | `feature/deep-eval-image-fix` | Show card image in Deep Evaluation results (`SingleCardResults` now renders front/back via base64 data URL); fix upgrade route: add `.rotate()` for EXIF orientation, size-limit to 2048px, use `_deep` / `_deep_back` storage paths to avoid CDN cache collision with original quick-scan crop |
...
- **Deep Evaluation image paths**: The original quick-scan multi-card crop is stored at `{userId}/{cardId}.jpg`. After a Deep Evaluation upgrade, the new full-resolution front is stored at `{userId}/{cardId}_deep.jpg` and back at `{userId}/{cardId}_deep_back.jpg`. Using different paths (instead of overwriting with upsert) avoids CDN cache collisions. The DB `image_path` / `back_image_path` columns are updated to point to these new paths on successful upload.
